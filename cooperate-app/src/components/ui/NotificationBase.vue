    ...
    <div class="col-sm-9">
      <div class="row">
        ...
        <div class="row box-description">
          ...
          <div>
            <p class="display-date">{{ getFormattedDate(creatAt) }}</p>
      ...
      <div class="row">
        <div class="col-sm-4 box-btn">
          <button-base link class="" :to="postDetailsLink" col="col-12"
...
export default {
  ...
  methods: {
    async deletePost() {
      var removeData = await this.$store.dispatch("admin/deletePost", this.id);
      if (removeData) {
        this.isShow = false;
        this.$toast.success(`Thông báo ${this.title} được xóa thành công.`);
      } else {
        this.$toast.error(`Thông báo${this.title} xóa không thành công.`);
      }
    },
